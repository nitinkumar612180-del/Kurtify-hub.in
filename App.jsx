...
// Every product/order/setting is stored as one document in the
// "kurtifyhub" Firestore collection, so it's shared/synced live between
// EVERY visitor — admin edits on one phone show up for every customer.
async function loadKey(key, fallback) {
  try {
    const snap = await getDoc(doc(db, "kurtifyhub", key));
...
async function saveKey(key, value) {
  try {
    await setDoc(doc(db, "kurtifyhub", key), { value: JSON.stringify(value) });
  } catch (e) {
